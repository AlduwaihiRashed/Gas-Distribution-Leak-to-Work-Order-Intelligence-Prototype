...
Visually it reads as a dark monitoring console — Nord-palette dark panels, frost-blue chrome, aurora-coloured grade badges, monospace HUD-style readouts on segment IDs/timestamps/KPI values — set via `.streamlit/config.toml` (dark theme base) plus the CSS block in `src/dashboard.py`. This is a styling layer only: it changes nothing about what data is shown or how grading/routing decisions are made.
